...
![tests](https://img.shields.io/badge/tests-12%2F12_passing-brightgreen)
...
|------|---------|--------------|
| Headless harness | `lua5.1 tests/run.lua` | Runs the pure-layer unit suites (classifier, ranker, selector, ID sentinels, schema, macro-body builders, bus, DebugLog formatters, tooltip parsing) plus a full-addon TOC-order load check against a `wow_mock.lua` stub of the WoW API — no client needed. |
