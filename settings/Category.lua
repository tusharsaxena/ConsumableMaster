...
-- ---------------------------------------------------------------------
-- Ownership + number formatting, for the priority rows
-- ---------------------------------------------------------------------
--
...
-- The two helpers settings/CategoryAddByID.lua borrows, published rather than
-- copied. That file owns the Add-by-ID line -- peeled out whole at layout-§1's
-- 1500-line cap -- and it draws its Type row with this page's `newRow` and
-- reports its writes through this page's `afterMutation`, so both halves of one
-- page agree about what a row looks like and what a mutation costs. It loads
-- immediately after this file and takes both as file-scope locals, which is why
-- the table is published HERE, at load, rather than built on demand.
KCM.Settings.MacrosPage = KCM.Settings.MacrosPage or {}
KCM.Settings.MacrosPage.NewRow        = newRow
KCM.Settings.MacrosPage.AfterMutation = afterMutation
...
local function renderSingle(ctx, cat)
    ...
    -- settings/CategoryAddByID.lua's, reached at RENDER time so this file has
    -- no load-order dependency on the one that loads after it.
    --
    -- GUARDED, like every other cross-file reach this peel created. The Add-by-ID
    -- line is ONE block on this page; a load that lost that file should cost the
    -- page that block, not raise index-of-nil and take the whole Macros tab with
    -- it. Unguarded, this was the one reach in the cut that could not degrade.
    local addByID = KCM.Settings.AddByID
    if addByID and addByID.Render then
        addByID.Render(ctx, scroll, cat, specKey)
    end
