...
local function assertVendorSync(tag, subdir, localDir, label)
    ...
    t.truthy(#shipped > 0, ("%s %s carries %s/"):format(label, tag, subdir))
    ...
    t.eq(table.concat(mine, ", "), table.concat(shipped, ", "),
    ...
    for _, name in ipairs(shipped) do
        ...
        t.truthy(blob ~= nil, ("%s %s carries %s"):format(label, tag, name))
        t.truthy(here ~= nil, ("the vendored copy carries %s"):format(name))
