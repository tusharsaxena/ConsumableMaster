-- tests/mock_menu.lua — not a suite: a headless stand-in for the client's
-- context-menu API (11.0+), for tests/test_launcher.lua and tests/test_disabled.lua.
--
-- COPIED FROM LibKa0s v1.58.0's tests/mock_menu.lua, body unchanged. The library
-- keeps it repo-local rather than in the kit (its CHANGELOG, "What a consumer owes
-- on re-vendoring v1.58.0": a host test of the menu needs a `MenuUtil` fake in the
-- host's own harness), so it is not under tests/_kit/ and test_vendor_sync does
-- not compare it. It is the library's fake on purpose: the same fidelity rules
-- the library's own cases rest on (a grayed entry is never clicked; the generator
-- runs once per open), so a case here cannot pass against a kinder menu than the
-- one Launcher.lua was tested against.
--
-- This harness resolves client globals through `_G` (the loader's env falls back
-- to it), so callers hand it `_G` as the `mocks` table and MUST `remove()` it when
-- done: mock.install() does not reset `MenuUtil`, and a leftover would change the
-- right click of every later case.
--
