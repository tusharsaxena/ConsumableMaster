...
local function glob(pattern)
  local out, p = {}, io.popen("ls -1 " .. ROOT .. "/" .. pattern .. " 2>/dev/null")
  if not p then return out end
  local prefix = ROOT .. "/"
  for line in p:lines() do
    if line ~= "" then out[#out + 1] = (line:sub(1, #prefix) == prefix) and line:sub(#prefix + 1) or line end
  end
  p:close()
  return out
end
...
local DEVIATION_ID = { "%f[%w]CM%-[%u%-]*%d+", "%f[%w]CONSUMABLEMASTER%-[%u%-]*%d+" }
