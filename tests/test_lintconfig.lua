...
--
-- WHAT IT PROVES. That `luacheck .` reaching 0/0 is a statement about the code and not about the
-- configuration. Three things are checked, and all three are the same rule seen from different
-- sides:
--   1. `.luacheckrc` sets no top-level `ignore`, and switches no warning class off wholesale.
--   2. Every `ignore` it does set sits in a `files[...]` stanza that is narrow — the key names one
--      `.lua` file, or the entry names the variable as well as the code (`212/self`).
--   3. No tracked `.lua` file carries a bare inline `-- luacheck: ignore` with no code after it,
--      which is the same blanket wearing a different hat.
--
-- WHY IT EXISTS, in this repo specifically. `.luacheckrc:25` carried `ignore = { "212", "542" }`
-- through the whole 2026-09-07 remediation, and the exit pass found it: `M4-11`'s rule is explicit
-- that a blanket ignore reads as coverage and provides none, and that is exactly what it did here.
-- The two codes were real — unused `self` on widget methods, one deliberately empty CSV branch —
-- but the suppression reached all 99 files, so a genuinely dead argument written into
-- core/BagScanner.lua tomorrow would have landed green with the badge still saying 0/0. Removing
-- the line is one afternoon's work. Keeping it removed is what this is for, and `M4c-02` is the
-- standing proof in this repository that a sweep without a gate comes back within four commits.
--
-- WHY IT READS THE CONFIG AS LUA RATHER THAN AS TEXT. `.luacheckrc` is a Lua chunk, and a text
-- scan for `ignore` would be defeated by any of the half-dozen ways Lua has of writing the same
-- assignment. Loading it under a sandboxed environment gives the same table luacheck itself builds,
-- so what this gate reads is what luacheck obeys. The environment auto-creates a table on first
-- index because `files["tests/"] = { ... }` is written without declaring `files` first, which is
-- how luacheck's own config loader behaves.
--
...
-- at the top level silences a whole family across the whole repository, which is `ignore` by
-- another spelling — the reason the list is here and not just `ignore`.
...
--- preceding `files = {}`, and inherits `_G` for the handful of standard functions a config may
--- legitimately call. Nothing is written back to `_G`: assignments land in the sandbox and are
--- what this gate then inspects.
...
test("lintconfig: .luacheckrc sets no top-level ignore", function()
    ...
    if ignore ~= nil then
        ...
        fail(".luacheckrc sets a top-level `ignore` of { " .. table.concat(shown, ", ") .. " }. "
            .. "A blanket ignore silences the code in all 99 files, including the ones with no "
...
-- Vendored trees are not ours to annotate, and this file quotes the forbidden directive in order
-- to forbid it.
...
test("lintconfig: no source file carries a bare inline luacheck ignore", function()
    ...
    for _, path in ipairs(trackedLua()) do
        ...
        for line in fh:lines() do
            ...
            -- is the form this rule is steering people towards.
