...
2. **Line counter.** The bottom-right label reads **`N / 1500 lines`** and **N climbs on every appended line**. Click **Clear** — the log empties and the counter resets to **`0 / 1500 lines`**.
...
4a. **Test mode.** Lock the bar, set General → **General visibility** to *Never*, and tick General → Master controls → **Test mode**. Chat prints `Test mode on — …`, and the bar comes up anyway, where it last sat, with its gold wash and the **Consumable Master** handle. Dragging the handle does nothing while **Lock frame** is on (its tooltip says to unlock), and clicks still pass through the gaps between buttons. Untick Lock frame → the handle drags the bar; tick it again. Set Macro Bar → Visibility → **Combat visibility** to *Only in combat* → the bar stays up out of combat. Untick all 15 on Macro Bar → **Buttons** → the bar lays out all 15 slots instead of collapsing to an empty cell. Untick **Test mode** → chat says `Test mode off` and the bar goes back to what the settings say (hidden, here). Tick it again and pull a mob: the moment combat starts, chat says `Test mode off — combat started`, the bar is back under its real visibility, and the box is unticked, with no taint error. Mid-fight, tick the box → one gray line saying test mode cannot start during combat, and the box stays unticked. Out of combat, `/cm bar off`, then tick it → one gray line telling you to turn the bar on with `/cm bar on`, and the box stays unticked. `/cm bar on`; `/cm test` toggles test mode and `/cm test on|off` sets it, `/cm bar test` and `/cm bar test on|off` do the same, and the box follows each. `/cm test` mid-fight gets the same gray combat line as the box. With it on, **Reset all settings** → Yes ends it. With it on, `/reload` → it is off. Put General visibility, Combat visibility and the Buttons tab back afterwards.
...
15a. **The collection's marks, and the one argument that decides whether they draw.** This is new and it is the check most likely to pass by accident, because every failure mode here is a perfectly good control that raises nothing.
   ...
   Open the console (`/cm debug`) and look at its title bar. The three controls to the right of the `Debug: ON/OFF` toggle must be **small square icons** — a copy mark, a clear mark and a close mark, all in the same light gray, all brightening on hover. Then click **copy** and look at the copy window's title bar: the same close mark. Then unlock the macro bar (`/cm bar unlock`) and look at the right end of the handle strip: the help control there is now the collection's **help mark**, not Blizzard's blue-and-white `InformationIcon`. Its multi-line tooltip is unchanged and must still list every drag gesture.
...
| Test mode (`MacroBar.SetTestMode`, the `state.testMode` row, `/cm test`, `/cm bar test`) | §11a step 4a, plus §7 step 4 (the row's place on Master controls) |
