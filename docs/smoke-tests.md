...
Run this after any change under `libs/LibKa0s/`, or to `core/CoreSetup.lua`, `modules/DebugLog.lua`, `core/SlashCommands.lua`, `modules/PerfSetup.lua` or `settings/Panel.lua`'s seam. Everything below is chrome, timing or frame behavior — the parts the headless harness provably cannot reach (the mock's `IsShown` always reads truthy, `HookScript` is a no-op, and named frames are never published to `_G`).
