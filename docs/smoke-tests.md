...
5a. **Suggestions while typing.** Hover the box first: the tooltip says to type a name and pick it from the list, and ends with the same hint. On a tab listing a crafted potion with ranks (for example, three ranks of one potion seeded or discovered there), type two or more letters of its name. Within a moment a list opens under the box, one row per match: icon, name, then the gray ID. Each rank is its own row, and the ranks are told apart by their IDs. Up/Down moves the highlight. Enter, or a click on a row, adds that rank: the row appears in the priority list and the box and the list clear. Type the full shared name and press Enter without picking: nothing is added, the status line reads `Several items share the name '<name>': pick one from the list, or use the ID.`, and the ranks list opens to pick from. Press Enter again without picking: still nothing is added. Escape closes the list. Now carry two ranks of a potion this tab does not list (for example, two ranks of *Potion of the Hushed Zephyr*, which no category seeds), type its full name and press Enter: nothing is added, the same shared-name line reads, and the list opens with the two carried ranks. A tab-listed rank and a carried rank list together. The rows show icon, name and gray ID but no crafted-quality tier icon or quality color, so the ranks are told apart by ID. Switch **Type** to Spell with text in the box: the page redraws, the text is still in the box, and typing more lists the tab's spells, not its items. On a spec-aware tab with no active spec, typing opens no list, and the box's tooltip offers none: it says a spec is missing and ends with no name hint.
