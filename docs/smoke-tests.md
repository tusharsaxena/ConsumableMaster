...
9b. **Whole-value rows (#35).** `/cm get macroBar.order` prints the slot keys in order. `/cm set macroBar.order DRINK,FOOD` moves Drink and Food to the front of the bar and keeps every other slot. `/cm set macroBar.shown FOOD=off` hides the Food button, and the Buttons tab's Food checkbox unticks. `/cm set categories.BATTLE_REZ.mouseover off` unticks **Cast on mouseover** and the Battle Rez body drops `[@mouseover,help]`; `/cm reset categories.BATTLE_REZ.mouseover` puts it back. `/cm stat primary AGI` then `/cm get statPriority` shows `<spec>: AGI > …`, and `/cm set statPriority x` is refused and names `/cm stat`. `/cm reset categories.HP_AIO.orderInCombat` puts Healthstone back above Healing Potion on the AIO Health tab. With `/cm debug on`, each panel control that writes one of these rows logs exactly one `[Set]` line for it: the AIO Health tab's Enabled checkbox, a drag between its rows and its Reset category, the Stat Priority page's list and Defaults, the Buttons tab's checkboxes, a drag on the bar, and Reset all priorities.
...
16. **Page Defaults.** With the bar heavily customized, press the top-right **Defaults** button on the Macro Bar page → every bar setting, its position, slot order and per-macro visibility go back to shipped values, and no other page is touched. **Lock frame** on the General page keeps its state, and with `/cm debug on` the console shows one `[Set] macroBar.<field>` line per row the page owns.
