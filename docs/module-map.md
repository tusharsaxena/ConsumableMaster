...
core/Diagnostics.lua  The diagnostics report's sections (debug-logging-§14,
                   DX-CM): state, settings, spec, tooltip cache (read before
                   any ranking touches it), categories, weapon enchant, macros,
                   macro bar, bags, events. Read-only;
...
| `core/Diagnostics.lua` | The diagnostics report's sections (`debug-logging-§14`, DX-CM), published as `KCM.Diagnostics.Sections()`: state (stored switch, stood down, holds, schema, perf), settings (`out:nonDefaults` over the schema rows, with `enabled`, `macroBar.enabled`, `macroBar.locked` and `global.minimap.shown` always printed), spec and stat priority, the tooltip cache (read before the categories, whose ranking goes through `TooltipCache.Get` and can resolve a pending entry), per category the stored edits, the top five of the effective priority with score and owned flag and the pick the macro carries, the weapon-enchant slots, every managed macro and the combat queue, the macro bar, the bags and the client events. Read-only: it never calls `Selector.PickBestForCategory` (whose level gate asks `IsUsableByPlayer` of pending entries) or `GetBucket` on a spec bucket that does not exist yet. `core/DebugLogSetup.lua`'s descriptor asks for the sections at run time, and the library writes the markers, the identity header, the per-section pcall and the cap. |
