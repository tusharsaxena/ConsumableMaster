...
core/Compat.lua ── Spec + spell API seam. GetSpecialization /
                   ...
                   GetSpecializationInfoForClassID stay host code. IsSecret
...
| `core/Compat.lua` | Spec + spell API seam, and the addon's half of `LibKa0s-Compat-1.0`. `KCM.Compat.GetSpecialization / GetSpecializationInfo / GetSpellName / IsSecret` are the major's members bound onto the table (v1.55.0), so every caller still names `KCM.Compat.X`; `GetNumSpecializationsForClassID` (answers `0` when no API does, because its callers loop `1..n`) and `GetSpecializationInfoForClassID` stay host code, this addon being their only consumer. `GetSpellName` answers `nil` on a miss so each call site picks its own placeholder: `modules/KCMItemRow.lua:85` shows `[Loading]`, and `modules/MacroBarFlyout.lua` writes `""` into a secure `spell` attribute, where `nil` would leave the previous binding in place. `IsSecret(value)` wraps the client's `issecretvalue` (false on a pre-Midnight client) so a gate over client data can ask before it compares — see [midnight-quirks.md](./midnight-quirks.md#secret-values). With the major absent the readers answer `nil` and `IsSecret` keeps its one-rung body (`options-ui-§1`). SpecHelper / SlashCommands / MacroManager / MacroDisplay / settings / KCMItemRow all route through it. |
