...
| `core/MacroDisplay.lua` | Read-only display resolution for a KCM macro: `PickID / Texture / TextureForID / Count / Cooldown / SetTooltip / MacroIndex` + `FALLBACK_ICON`. Reads the pick MacroManager recorded in `db.profile.macroState`; calls no protected API. Shared by the macro bar's slots and `modules/KCMMacroDragIcon.lua` so icon/tooltip resolution can't drift between them. |
