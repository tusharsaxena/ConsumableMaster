...
| `core/Constants.lua` | `KCM.PREFIX` (cyan `[CM]` tag) + `KCM.Say`. Single source of truth for chat-output styling. |
...
| `core/Debug.lua` | `KCM.Debug.IsOn() / Toggle() / Print(fmt, ...) / Log(fmt, ...)`. Reads the session-only `KCM.State.debug`; `Toggle` routes through `KCM.DebugLog:SetEnabled`. Diagnostics go to the on-screen console; chat is a fallback only when the console is absent. |
...
| `modules/DebugLog.lua` | On-screen debug console — `ConsumableMasterDebugWindow` + `ScrollingMessageFrame`, JetBrains Mono via LibSharedMedia, Copy / Clear / Toggle header controls. `SetEnabled / IsEnabled / Toggle / AddLine / Show / Hide` + pure `FormatPlain / FormatColored`. |
...
- `tests/` — headless harness (`lua5.1 tests/run.lua`, 9 suites) over the pure layer; `wow_mock.lua` stubs the WoW API + bus.
