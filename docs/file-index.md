...
| `core/Debug.lua` | `KCM.Debug.IsOn() / Toggle() / Print(fmt, ...) / Log(tag, fmt, ...)`. Reads the session-only `KCM.State.debug`; `Toggle` routes through `KCM.DebugLog.Toggle` → `SetEnabled`. Diagnostics go to the on-screen console; chat is a fallback only when the console is absent. |
...
- `tests/` — headless harness (`lua5.1 tests/run.lua`, 10 suites) over the pure layer; `wow_mock.lua` stubs the WoW API + bus.
