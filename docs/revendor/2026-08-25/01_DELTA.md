# 01 — Delta: ConsumableMaster vs LibKa0s v1.15.0
...
```sh
grep -in 'bundles' ConsumableMaster/CLAUDE.md
```
...
```sh
grep -hoE 'local +(MAJOR, )?[A-Z_]*MINOR *= *("[^"]+", *)?[0-9]+' ConsumableMaster/libs/LibKa0s/*.lua
```
...
```sh
diff -r --strip-trailing-cr <scratch>/LibKa0s ConsumableMaster/libs/LibKa0s   # 0 lines
diff -r                     <scratch>/LibKa0s ConsumableMaster/libs/LibKa0s   # 0 lines
diff -r --strip-trailing-cr <scratch>/testkit ConsumableMaster/tests/_kit     # 0 lines
diff -r                     <scratch>/testkit ConsumableMaster/tests/_kit     # 0 lines
```
...
```sh
grep -rnoE 'LibStub\("LibKa0s-[A-Za-z]+-1\.0", true\)' ConsumableMaster --include='*.lua' | grep -v '/libs/' | grep -v '/tests/'
```

- `ConsumableMaster/core/CoreSetup.lua:33` — Core
- `ConsumableMaster/core/DebugLogSetup.lua:59` — DebugLog
- `ConsumableMaster/core/EnvSetup.lua:57` — Env
- `ConsumableMaster/core/MediaSetup.lua:61` — Media
- `ConsumableMaster/core/PerfSetup.lua:38` — Perf
- `ConsumableMaster/settings/OptionsSetup.lua:69` — Options
- `ConsumableMaster/settings/Slash.lua:257` — Slash
...
```sh
grep -n 'Kit.VERSION' <scratch>/testkit/framework.lua ConsumableMaster/tests/_kit/framework.lua
```
...
| Gate | Command | Result |
|---|---|---|
| Lint | `luacheck .` | 0 warnings / 0 errors in 58 files |
| Tests | `lua tests/run.lua` | 698 passed, 0 failed, 0 skipped |
