...
```
grep -n '[Bb]undles' ConsumableMaster/CLAUDE.md
```
...
```
diff -r --strip-trailing-cr <tag>/LibKa0s ConsumableMaster/libs/LibKa0s   # 1 file: OptionsCompose.lua
diff -r --strip-trailing-cr <tag>/testkit ConsumableMaster/tests/_kit     # empty
```
...
Majors reached from this addon's own source: Core DebugLog Env Item Media Options Perf Slash Widgets 
