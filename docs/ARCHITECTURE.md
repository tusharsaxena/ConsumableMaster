...
**Pending an upstream ruling.** The standard contradicts itself on this point. The v2.43.0 changelog entry in `STANDARDS.md` lists ConsumableMaster among the addons that "hold no registry and owe nothing". But the three tests in the rule text classify these id sets as a registry, and "id sets" is one of the rule's own examples. This section follows the rule text. The changelog's survey appears to have missed the `added` / `blocked` sets, and a correction to that roll-call is proposed upstream. Until the standard rules, treat this classification, and the "not yet the only runtime writer" finding below that depends on it, as provisional. The list under "Other state written outside the helper" rests on the schema-row MUST and the MUST NOT, which predate v2.43.0, so it stands either way.
...
- `profile.macroState[macroName]`, `MacroManager`'s per-macro fingerprint cache (`lastItemID` / `lastBody` / `lastIcon` / `lastCat`). It is created lazily at `modules/MacroManager.lua:465`, written per macro at `:486`, and wiped to `{}` by `InvalidateState` (`:636`, reached from `/cm rewritemacros` and the General page's Force rewrite macros button). It is not a preference, so it wants a `Documented deviations` row rather than a schema row. debug-logging-§10 keeps such state out of `[Set]` logging, but it does not exempt it from the MUST NOT. The combat queue `pendingUpdates` lives only in memory, so it is out of scope;
