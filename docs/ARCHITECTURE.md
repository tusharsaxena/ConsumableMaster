...
- **The macro bar's position.** Storage key: `profile.macroBar.point` / `.relPoint` / `.x` / `.y`, the bar's anchor against `UIParent`. It is geometry only a drag determines. **Owner:** `MacroBar` (`modules/MacroBar.lua`). **Writers:**
  - `savePosition` (`:101`, write at `:107`) reads the anchor back off the frame when a drag stops, on the bar itself (`OnDragStop`, `:143-146`) or on its drag handle, which since LibKa0s v1.48.0 hands the same function to the widget as its `onDragStop` (`:192`).
  - `MB.ResetPosition` (`:541`, write at `:545`) puts back the shipped default from `KCM.dbDefaults`. Three acts reach it: the General page's **Reset position** button (`onResetPosition`, `settings/General.lua:216`), `/cm bar reset` (`core/SlashCommands.lua:939`) and the Macro Bar page's **Defaults** (`doResetPage`, `settings/MacroBar.lua:635`).
