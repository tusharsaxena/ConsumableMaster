...
- **English-only.** Classifier compares subType against literal strings (`"Potions"`, `"Food & Drink"`, `"Flasks & Phials"`) and TooltipCache patterns are English. If Blizzard renames a subType, edit the `ST_*` constants in `Classifier.lua`. `locales/enUS.lua` is a shell, not localization plumbing.
