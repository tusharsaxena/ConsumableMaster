...
**Selector is the only runtime writer.** `architecture-§5` makes a registry reset a writer operation, and all three reset doors call the writer: `/cm priority <cat> reset` calls `Selector.ResetBucket` (`core/SlashCommands.lua:408`), the Macros page's per-category reset popup calls it for the viewed spec (`settings/Category.lua:190`), and **Reset all priorities** calls `Selector.ResetAllBuckets` (`settings/General.lua:124`). Until 2026-09-12 each of the three cleared the buckets itself, which spread registry writes across three runtime modules. anti-patterns #78 names that as a code finding. It was tracked in [#34](https://github.com/tusharsaxena/ConsumableMaster/issues/34) and is closed. `tests/test_selector.lua` now fails if any file other than `modules/Selector.lua` assigns a bucket's `added`, `blocked` or `pins`. Reset all priorities also clears `statPriority`. That write is a setting, not registry membership, and it goes through the schema helper like the other `statPriority` writers (below).

### Other state written outside the helper

None of the following is a registry. Each is a preference, an order over a fixed set of shipped keys, or other persistent state such as a cache or drag geometry. `architecture-§5` wants a schema row or a `Documented deviations` row for each preference or order, and its MUST NOT wants a `Documented deviations` row for the other state. As of 2026-09-12 none has either, and that is an open finding, tracked in [#35](https://github.com/tusharsaxena/ConsumableMaster/issues/35) (the drag geometry in [#37](https://github.com/tusharsaxena/ConsumableMaster/issues/37)):
...
- the composite sections `profile.categories[HP_AIO|MP_AIO].enabled[ref]`, `.orderInCombat` and `.orderOutOfCombat`, written by `Selector.MoveCompositeRef` (`modules/Selector.lua:646`), `/cm aio` (`core/SlashCommands.lua:704`, `:720`, `:733-735`) and the composite page (`settings/Category.lua:180`, `:899`);
...
- `profile.macroState[macroName]`, `MacroManager`'s per-macro fingerprint cache (`lastItemID` / `lastBody` / `lastIcon` / `lastCat`). It is created lazily at `modules/MacroManager.lua:465`, written per macro at `:486`, and wiped to `{}` by `InvalidateState` (`:636`, reached from `/cm rewritemacros` and the General page's Force rewrite macros button). It is not a preference, so, reading the MUST NOT's "any other persistent state" literally, it wants a `Documented deviations` row rather than a schema row. debug-logging-§10 keeps such state out of `[Set]` logging, but it does not exempt it from the MUST NOT. The combat queue `pendingUpdates` lives only in memory, so it is out of scope;
