...
| Folder | Holds |
|--------|-------|
| `core/` | Namespace, AceAddon entry (`ConsumableMaster.lua`) + recompute pipeline, Bus, Compat, Constants, State, Database, Debug, the pure engine (SpecHelper, TooltipCache, BagScanner, Classifier, WeaponSlots), the macro bar's pure halves (MacroDisplay, MacroBarModel, MacroBarLayout), the `/cm dump` targets (SlashDump), the slash **verb bodies** (SlashCommands) and the diagnostics report's sections (Diagnostics) — the dispatcher itself is `settings/Slash.lua` |
...
Two things are deliberately *not* schema rows. `KCM.State.debug` — the session LOGGING flag — is never persisted, so it has no path to declare; the Master controls tab's `Debug console` row is a different thing, the console WINDOW's visibility, and it declares the session path `state.debugConsole`, whose store is the row's own `get` / `set` (`settings/General.lua`). The per-category item lists are a collection no row shape describes: they are the structural registry (below), which is also why `/cm resetall` stays host-owned rather than adopting the library's `Sl:CliResetAll` (closed issue [LIBKA0S-12](https://github.com/tusharsaxena/ConsumableMaster/issues/27)). The per-spec stat priorities used to be the same kind of exception, and `architecture-§5` treats them differently. They are a preference, and a preference with no row is a missing row. Since 2026-09-12 they are one whole-value row, `statPriority` (below).

### Structural registry: the per-category item lists

This addon holds **one** structural registry (`architecture-§5`). The player adds and removes its members at runtime, `dbDefaults` ships it empty (`defaults/Profile.lua:75-109`), and no schema row names a member.
...
- **Load pass.** `KCM.Database.RunMigrations` (`core/Database.lua:182`), called from `KCM:OnInitialize` (`core/ConsumableMaster.lua:41`) and from the three AceDB profile callbacks that `KCM.RegisterProfileCallbacks` installs (`core/ConsumableMaster.lua:445-508`). It has no `categories` step today: AceDB's merge of `dbDefaults` is what seeds the empty buckets.

**Selector is the only runtime writer.** `architecture-§5` makes a registry reset a writer operation, and all three reset doors call the writer: `/cm priority <cat> reset` calls `Selector.ResetBucket` (`core/SlashCommands.lua:442-443`), the Macros page's per-category reset popup calls it for the viewed spec (`settings/Category.lua:288-289`), and **Reset all priorities** calls `Selector.ResetAllBuckets` (`settings/General.lua:129-130`). Until 2026-09-12 each of the three cleared the buckets itself, which spread registry writes across three runtime modules. anti-patterns #78 names that as a code finding. It was tracked in [#34](https://github.com/tusharsaxena/ConsumableMaster/issues/34) and is closed. `tests/test_selector.lua` now fails if any file other than `modules/Selector.lua` assigns a bucket's `added`, `blocked` or `pins`. Reset all priorities also clears `statPriority`. That write is a setting, not registry membership, and it goes through the schema helper like the other `statPriority` writers (below).
...
- **The macro fingerprint cache.** Storage key: `profile.macroState[macroName]` = `{ lastItemID, lastBody, lastIcon, lastCat }`, the fingerprint of the last edit `MacroManager` made to each macro, which its "unchanged" early-out compares against. It is learned data: every entry records an edit the addon made, and the player authors none of them. **Owner:** `MacroManager` (`modules/MacroManager.lua`). **Writers:**
  - `commitMacro` (`:458`) stores a macro's fingerprint (`:493`) after a successful `EditMacro`. It is the one write seam behind `SetMacro`, `SetWeaponEnchantMacro` and `SetCompositeMacro`. A pipeline recompute reaches it, and so does `FlushPending` when the combat queue drains on `PLAYER_REGEN_ENABLED`, which replays each queued single-category body through it rather than rebuilding one. Its `macroState = macroState or {}` (`:472`) only creates the empty table on first use, which the rule does not count as a writer.
  - `InvalidateState` (`:661`, write at `:668`) clears the whole cache so the next pass rewrites every macro. It is a forced rebuild, one of the owner's own operations. Three acts reach it: `/cm rewritemacros` (`settings/Slash.lua:244`), the General page's **Force rewrite macros** button (`doForceRewriteMacros`, `settings/General.lua:101`), and a profile switch or copy, through the profile handler (`KCM.RegisterProfileCallbacks`, `core/ConsumableMaster.lua:445`). The incoming profile's fingerprints describe bodies the account's macros may no longer hold, so the handler clears them before its resync ([profiles.md](./profiles.md#why-the-fingerprints-are-forgotten)).
...
- **The macro bar's position.** Storage key: `profile.macroBar.point` / `.relPoint` / `.x` / `.y`, the bar's anchor against `UIParent`. It is geometry only a drag determines. **Owner:** `MacroBar` (`modules/MacroBar.lua`). **Writers:**
  - `savePosition` (`:101`, write at `:107`) reads the anchor back off the frame when a drag stops, on the bar itself (`OnDragStop`, `:155-158`) or on its drag handle, which since LibKa0s v1.48.0 hands the same function to the widget as its `onDragStop` (`:204`).
  - `MB.ResetPosition` (`:572`, write at `:576`) puts back the shipped default from `KCM.dbDefaults`. Three acts reach it: the General page's **Reset position** button (`onResetPosition`, `settings/General.lua:216`), `/cm bar reset` (`core/SlashCommands.lua:939`) and the Macro Bar page's **Defaults** (`doResetPage`, `settings/MacroBar.lua:635`).
...
`KCM.bus` is an AceEvent-embed, the publisher. **Every receiver owns its own target** via `KCM.NewBusTarget()`; two subscriptions never share one table. Since v1.55.0 the stand-down record behind those targets is `LibKa0s-Bus-1.0`'s: `core/Bus.lua` builds one record (`KCM.busRecord = Bus:New{ name, isDown = KCM.IsStoodDown }`), every target `NewBusTarget` hands out is tracked by it, and `KCM.NewBusTarget(subscribe)` keeps its optional subscribe function as a shim that runs once on the new target. `KCM.MSG` names each message, and is built by the major's `Catalog`, which validates the `Ka0s_ConsumableMaster_<Event>` names at load and answers a strict table: reading an undeclared key raises. With the major absent `core/Bus.lua` takes the untracked-target stub (`options-ui-§1`): private targets, no record, and `KCM.MSG` is the plain table.
...
Every client event this addon listens to is declared in one place — the `KCM.EVENTS` list of `{ event, handler }` pairs in `core/ConsumableMaster.lua` — and registered by `KCM:OnEnable`, which walks it through AceEvent. Each registration goes through `KCM.SafeRegisterEvent` (`LibKa0s-Core-1.0`'s `SafeRegisterEvent`, bound in `core/CoreSetup.lua`; the degraded stub keeps its one-rung `pcall` body), so a name the client refuses is appended once to `KCM.RejectedEvents` and skipped instead of raising and leaving every later event unregistered (events-frames-taint-§1). The stand-down's combat re-arm of `PLAYER_REGEN_ENABLED` (`core/LifecycleSetup.lua`) and the settings bootstrap frame (`settings/Panel.lua`) go through the same helper with the same list. A player reaches the list two ways: `/cm dump events` prints each `KCM.EVENTS` name as `registered`, `rejected` or `off`, and the `[Init]` debug summary ends `, rejected events: …` when the list is non-empty ([midnight-quirks.md](./midnight-quirks.md#learned_spell_in_tab-removed-in-retail)). No module body subscribes on its own, so the whole surface is readable at a glance. That single list is also what the stand-down tears down and what the stand-up rebuilds: `standDown` drops the lot with `KCM:UnregisterAllEvents()` and `standUp` **calls** `OnEnable` rather than copying its list, so the two can never name different sets ([the disabled state](#the-disabled-state-is-total)). `standUp` then runs login's discovery pass and stale sweep, `KCM.Pipeline.DiscoverAndSweep("stand_up")`, before its recompute: `PLAYER_ENTERING_WORLD` does not fire again on a re-enable, and a consumable looted while the addon was down has to be a candidate on the way back up, not at the next `BAG_UPDATE_DELAYED`. `OnEnable` itself refuses to run while a hold is taken, so a player who logs in with the addon off registers nothing at all.

| Event | Handler | Purpose |
|---|---|---|
| `PLAYER_ENTERING_WORLD` | `OnPlayerEnteringWorld` (`:605`) | Login and `/reload`: auto-discovery, then the discovered-set sweep (the two are `discoverAndSweep`, which the stand-up shares), then the first recompute, then `MacroBar.Update()` — in that order, because each step feeds the next |
| `BAG_UPDATE_DELAYED` | `OnBagUpdateDelayed` (`:625`) | Bag contents moved; re-run discovery and request a coalesced recompute |
| `PLAYER_SPECIALIZATION_CHANGED` | `OnSpecChanged` (`:630`) | Recompute the spec-aware picks and publish `SPEC_CHANGED` for the Stat Priority page |
| `PLAYER_REGEN_ENABLED` | `OnRegenEnabled` (`:640`) | Combat ended: flush MacroManager's pending macro writes and the macro bar's deferred build / relayout / restyle. A settings-category registration parked in combat is `LibKa0s-Options-1.0`'s to replay, on its own frame |
| `GET_ITEM_INFO_RECEIVED` | `OnItemInfoReceived` (`:669`) | Item metadata arrived: invalidate that item's cache entry, then a full recompute only if it is a bag item — everything else takes the debounced `PANEL_REFRESH` path instead |
| `LEARNED_SPELL_IN_SKILL_LINE` | `OnLearnedSpell` (`:690`) | A spell-backed candidate became known after the spell book hydrated; recompute |
| `PLAYER_EQUIPMENT_CHANGED` | `OnEquipmentChanged` (`:698`) | Recompute on main-hand (16) / off-hand (17) swaps only — the per-hand `WPN_ENCH` pick; every other slot is a no-op |
| `SPELL_UPDATE_COOLDOWN` | `OnCooldownUpdate` (`:619`) | Repaint macro-bar and flyout cooldown swipes. Bar-only, with an early-out when the bar is disabled |
| `BAG_UPDATE_COOLDOWN` | `OnCooldownUpdate` (`:619`) | The same repaint, from the item-cooldown side |
...
|---|---|
| Every registration on a bus target, actually unregistered | `KCM.Bus.StandDown()` — the `LibKa0s-Bus-1.0` record tracks every target `KCM.NewBusTarget` hands out, with or without a subscribe function, and takes its messages and events down; `KCM.Bus.StandUp()` replays the record as it is **now**, and a registration made while down goes live only then. A bare `StandUp` while a hold is still taken is refused (`core/Bus.lua`) |
...
| The bar hidden **at the source**, its fade `OnUpdate` cleared, its secure visibility driver unregistered and every flyout's `kcmCombat` attribute driver unregistered | `MacroBarModel.IsEnabled()` answers false while the latch is down, so `MB.Update` takes its own disable path, which calls `MacroBarFlyout.StandDown()`; the enable path re-arms the same flyout frames through `MacroBarFlyout.StandUp()`. Hiding imperatively would last until the next combat transition or settings change re-showed it |
...
| The settings-category registration, including one parked in combat (LibKa0s-Options-1.0 replays it on its own frame, whatever the stand-down state), and the panel body | The *Enable Consumable Master* checkbox is live there in either state |
...
**The launcher's clicks** follow `launcher-§2` (the standard's v2.67.0, drawn by `LibKa0s-Launcher-1.0` minor 4): the left click opens the settings panel in either state, and the right click opens the options menu — **Enabled** (`/cm enable|disable`'s handler, `Verbs.SetEnabled`) and **Locked** (`/cm lock|unlock`'s, `Verbs.RunLock`), the two entries the standard's `ADDONS.md` records for this addon. While disabled the library grays **Locked** with `enable the addon first` and a click on it reaches no handler and writes no SavedVariables; **Enabled** stays live, so the menu is one of the routes back on. **Its tooltip keeps drawing while disabled** (`launcher-§1`, minor 3's block, drawn from the descriptor): `Enabled: No`, `Locked: Yes|No`, and the same two fixed hints, `Left-click: Open settings` and `Right-click: Options menu`; [settings-panel.md → the Minimap button row](./settings-panel.md#the-minimap-button-row--shown-says-one-thing-the-store-says-the-other) has the full shape.

`tests/test_disabled.lua` is the conformance suite `slash-commands-§7` requires. It asserts on the **registration set** through the kit's recording mock, never on a handler's return value — a suite that asserts "the handler returned early" certifies the draw gate it exists to catch.
...
- **Options registration is deferred in combat; opening is refused outright.** Both are `LibKa0s-Options-1.0`'s. `Settings.RegisterAddOnCategory` is protected, and registering under lockdown taints the Settings window for the rest of the session — so `settings/Panel.lua`'s `registerPanel` hands the whole surface to `UI.CreateOptionsPanel`, which under lockdown registers nothing, parks the request and replays it once on a library-private `PLAYER_REGEN_ENABLED` frame. The replay does not go through the addon, so it runs whatever the stand-down state: a registration parked while the addon is disabled still lands (ConsumableMaster-R-03). Nothing is lost by waiting, because the panel cannot be opened mid-fight anyway: `KCM.Options.Open` (`settings/OptionsShim.lua`) calls `UI.OpenOptionsPanel`, which turns a mid-fight `/cm config` into the library's one `COMBAT_REFUSED` chat line instead of a silent failure, and `Open` answers `false`. The panel's Defaults action (`settings/Panel.lua`'s `Helpers.CreatePanel`) refuses separately and in its own words, because it is declining a reset rather than an open.
...
- **Blizzard API churn goes through `core/Compat.lua`.** `KCM.Compat` wraps the spec, spell and item APIs Blizzard keeps renaming (`GetSpecialization*`, `GetSpecializationInfoForClassID`, spell-name lookup, `GetItemInfo`) and the client's `issecretvalue` (`Compat.IsSecret`). Four of those seven are `LibKa0s-Compat-1.0`'s members bound onto the table; the seam is still the only thing callers name. SpecHelper, SlashCommands, MacroManager, MacroDisplay, the Ranker, the tooltip cache, the Classifier, WeaponSlots, KCMItemRow and the settings pages call through `Compat.*` and never the raw global, so a rename is one edit. Any gate over client data a combat restriction could turn secret must ask `IsSecret` *before* comparing ([midnight-quirks.md](./midnight-quirks.md#secret-values)).
- **Reset is centralized, and it is a PROFILE reset plus a session sweep.** `KCM.ResetAllToDefaults(reason)` (`core/ConsumableMaster.lua`) restores every `sessionOnly` schema row to its default and then runs one `db:ResetProfile()` — the same act as AceDBOptions' Reset Profile (`options-ui-§12`), with the row sweep `options-ui-§12` also makes a MUST because a profile reset by construction cannot reach a row whose storage is its own `set()` (`state.debugConsole`, stamped in `settings/General.lua`). It is the only PROFILE reset path; the General page's **Reset all settings** button (the Master controls tab's closing pair, `options-ui-§15`) and `/cm resetall` both raise the one `KCM_CONFIRM_RESET` popup (`core/SlashCommands.lua`), whose `OnAccept` is its only caller, behind the collection's one confirmation wording. The combat refusal (`false, "combat"`, before any write) and the closing `RefreshAllPanels` repaint are inside the function too, so neither door adds anything the other lacks. Don't add a third door, or a second popup. **Reset all priorities**, on the General page's **Maintenance** tab beside Master controls, is a different and narrower act — every category's added/blocked/pinned items and every spec's stat-priority override, and nothing else — behind its own confirmation (`KCM.ResetAllPriorities`, `settings/General.lua`); the button used to carry that name and run the profile reset. The resync (`afterReset`: invalidate → discover → recompute) is **not** called from there: `OnProfileReset` reaches it through the profile callbacks `KCM.RegisterProfileCallbacks` installs, so it runs on exactly one path — the one a profile *switch* takes too. The same handler logs the act, once: `[Set] reset profile '<name>' to defaults: N rows` (and `[Set] copied profile 'A' → 'B'` for a copy; a switch rewrites no rows and logs `[Profile] switched to '<name>'` instead of a `[Set]` line). On a switch or copy it also forgets the macro fingerprints before the resync, and on all three it ends by publishing `PROFILE_CHANGED`, off which the macro bar re-applies itself whole and every open settings page rebuilds ([profiles.md](./profiles.md)). `ResetAllToDefaults` runs both halves inside `Helpers.MuteSetLog`, so the session sweep's write logs no row and the whole reset is one line (`debug-logging-§10`). The handler also silences any `Helpers.Bulk` bracket open around it (the seam's `ConsumeResetCount` marks the open bracket as a profile reset), so a reset or copy run inside one is still that one line. N is the rows the reset actually changed, counted by the seam's `ResetCounted` before `db:ResetProfile()` replaces the profile. AceDBOptions' own Reset Profile button gives no earlier hook, so its line carries no count. The hand-written reset function it replaced, which named `categories`, `statPriority` and `enabled` and nothing else, is gone with it: a list of profile keys is the shape that quietly stops being true. `/cm reset path` is unrelated — the library's one-row schema reset ([LIBKA0S-12](https://github.com/tusharsaxena/ConsumableMaster/issues/27)), which never touches `categories` or `statPriority` ([schema.md](./schema.md)).
...
- LibKa0s — the Ka0s-owned shared modules, vendored whole-folder from [github.com/tusharsaxena/LibKa0s](https://github.com/tusharsaxena/LibKa0s) and loaded through the library's own packaged XML. Fourteen majors are adopted: `Core-1.0` (chat printer), `DebugLog-1.0` (debug console), `Slash-1.0` (dispatcher, help rows and schema CLI), `Options-1.0` + its `OptionsWidgets` / `OptionsScroll` attachments (panel shell, row widgets, canvas contract), `Perf-1.0` + `PerfPanel` (A/B capture), `Media-1.0` (the shipped icon catalog and font), `Env-1.0` (the TOC-manifest reader behind `KCM.Meta` / `KCM.Version`), `Item-1.0` (one primitive, `ItemIDFromLink`), `Widgets-1.0` (two surfaces: `ReorderList`, behind the priority rows' drag handle, and `DragHandle`, the macro bar's unlocked strip, adopted at v1.48.0), `Launcher-1.0` (the minimap button and the broker plugin as one object, adopted at v1.39.0) and `Lifecycle-1.0` (the one latch the disabled state and the perf harness's suspended arm are two named holds on, adopted at v1.41.0), `Compat-1.0` (the spec and spell ladders and the secret guard behind `KCM.Compat`, adopted at v1.55.0), `Bus-1.0` (the stand-down record behind every bus target, and the strict message catalog, adopted at v1.55.0) and `Schema-1.0` (the settings write seam, adopted at v1.56.0, [#39](https://github.com/tusharsaxena/ConsumableMaster/issues/39)). `Pool` is vendored with the payload but not consumed. Never patched in place — a fix goes upstream, then re-vendors whole-folder ([testing.md](./testing.md)).

### LibKa0s adoption

Each adopted major has one host **setup file** that builds ONE instance via `lib:New(descriptor)`,
publishes the addon's flat, dot-callable names as forwarders onto it, and carries a degradation stub
for a missing library. The per-major table (host half, what the addon keeps), the three load-bearing
rules and the declined surfaces are [module-map.md → LibKa0s adoption](./module-map.md#libka0s-adoption).
...
6. `# Settings` — `OptionsSetup.lua` (must come first — the `LibKa0s-Options-1.0` seam; creates `KCM.Settings.Helpers` and publishes `KCM.Settings.optionsUI`) → `Panel.lua` (the schema half, `RegisterTab`, the panel bootstrap and `KCM.Options.Register`) → `OptionsShim.lua` (the run-time `KCM.Options` surface: `Refresh` / `RequestRefresh` / `Open`, the refresh debounce and the bus receivers; after `Panel.lua`, whose `Helpers`, `optionsUI` and panel-unavailable notice it takes as file-scope locals) → `General.lua` → `MacroBar.lua` → `StatPriority.lua` → `Category.lua` → `CategoryAddByID.lua` (the Add-by-ID line; after Category.lua, whose `KCM.Settings.MacrosPage` helpers it takes as file-scope locals) → `Slash.lua` → `Profiles.lua` (last, as the Profiles page is last in the sidebar)
...
- **A panel opened while disabled cannot hydrate item rows.** The settings panel survives the disabled state, but its two hydration triggers do not: the stand-down's `UnregisterAllEvents` (`core/LifecycleSetup.lua`) drops `GET_ITEM_INFO_RECEIVED`, and the options bus target's `PANEL_REFRESH` subscription (`settings/OptionsShim.lua`) goes down with it. So in a fresh session, an item row whose name is not cached stays `[Loading]` (`modules/KCMItemRow.lua`) until re-enable, when standUp's `RequestRecompute` refreshes the panel. slash-commands-§7 records the panel-refresh-while-disabled question as "Recorded, not ruled". Panel-owned hydration through LibKa0s-Item's `LoadItem`, which `core/ItemSetup.lua` declines today, is the owner-decision follow-up in [#40](https://github.com/tusharsaxena/ConsumableMaster/issues/40).
...
- **A build with no `LibKa0s-Bus-1.0` leaves its bus subscriptions live when disabled.** The stand-down record is the library's since v1.55.0, and `core/Bus.lua`'s library-absent stub is the untracked-target shape the Bus API document prescribes: receivers still get private targets and still subscribe, but nothing is recorded, so `KCM.Bus.StandDown` takes nothing down on that install. Every publisher of those messages is itself stood down, so what survives is registered but unreached. `core/CoreSetup.lua` already announces the missing payload.
...
| `module-map.md` | Every non-vendored file, its responsibility, and load order; the LibKa0s adoption table and the peel history |
...
| Doc | Status | Trigger |
|---|---|---|
| `slash-dispatch.md` | Present | Twenty-two verbs, over the eight-or-more threshold, and four of them carry a subcommand tree (`priority`, `stat`, `aio`, `bar`, plus the `dump` targets) |
| `midnight-quirks.md` | Present | Client-version workarounds of the addon’s own |
| `debug.md` | Present | The diagnostics report (`core/Diagnostics.lua`, debug-logging-§14) and the `/cm dump` targets in `core/SlashDump.lua` are the addon’s own beyond the library console |
| `message-bus.md` | Not applicable | Five messages; threshold is more than ten. The table lives in `ARCHITECTURE.md` → `## Message Bus` |
| `compat-layer.md` | Present | `core/Compat.lua` publishes three addon-specific shims (`grep -cE '^\s*function\s+[A-Za-z_][A-Za-z0-9_]*\.' core/Compat.lua` answers 3), at the three-or-more threshold since `GetItemInfo` joined the two class-ID spec readers. The other four are `LibKa0s-Compat-1.0`'s, which the library documents ([module-map.md → LibKa0s adoption](./module-map.md#libka0s-adoption)) |
...
| `compat` | Two sites read the item-count API without the Compat seam: `modules/KCMItemRow.lua:230` calls the `GetItemCount` global directly (guarded by `_G.GetItemCount`, with no `C_Item` rung), and `core/MacroDisplay.lua:76` keeps it as the fallback rung behind `C_Item.GetItemCount`. No other direct item-API global read remains: `GetItemInfo` is owned by `core/Compat.lua` (`KCM.Compat.GetItemInfo`, `C_Item.GetItemInfo` first, the global as a guarded fallback) since review finding `CM-R-05`, and every former direct site (`core/TooltipCache.lua`, `modules/Ranker.lua`, `modules/KCMItemRow.lua`, and the fallback rungs in `core/Classifier.lua` and `core/WeaponSlots.lua`) calls it | `compat`'s routing MUST is scoped to **deprecated** APIs, and `GetItemCount` is a live retail global. `C_Item.GetItemCount` answers the same count, so the row covers a missing first rung at one display-only call site (the Priority-list row's `[n]` prefix) and a fallback at the other, not a correctness gap. The `GetItemInfo` half of this row, and the reasoning that kept it direct (a live global needs no seam), was retired on 2026-09-24: whether a 12.x patch still ships that global is unverified, and it is read on every recompute and every tooltip-cache fill. Review findings `CM-R-10` and `CM-R-05`, audit findings `CM-63` and `CM-96` | 2026-08-05; narrowed 2026-09-24 | Blizzard deprecating the `GetItemCount` global, or a third direct item-count read being added |
| `options-ui-§1` | On a library-absent load, `/cm enable` and `/cm disable` take the WS-02 **route (b)**: they refuse with the one library-absent line (`/cm enable is unavailable: the LibKa0s library did not load.`), write nothing and raise nothing, instead of the SHOULD route (a), a `writeThrough` path on the Schema seam. `/cm lock`, `/cm unlock`, `/cm bar lock` and `/cm bar unlock` refuse the same way (the rule allows either route for them, so they are listed for completeness, not as a deviation). `/cm bar`, `bar on` and `bar off` are not composed-row verbs: `macroBar.enabled` is hand-declared in `settings/MacroBar.lua`, so on that build the write lands and the verb says so, and it prints its success line only when the seam's verdict is true (`core/SlashCommands.lua` `setBarShown`) | A library-absent build has no stand-down latch (`core/LifecycleSetup.lua` returns early), so a stored switch would not be honored as the switch it is this session: the one runtime reader outside the latch, `macrosEnabled()` in `core/ConsumableMaster.lua`, would skip the macro write pass while the bar, the events and the bus stayed up, which is the half-down state anti-pattern #85 names. Acknowledging that write would be the false success the rule forbids. The `enabled` row itself is composed by the library, so on this build no row exists for the seam to validate, and the stub is handed no `writeThrough` list (`settings/SchemaStub.lua`) | 2026-09-24 (CM-18, WS-02 ruling) | A library-absent Lifecycle latch stub, which would make a stored switch whole on this build; or `enabled` gaining a runtime reader outside the latch beyond `macrosEnabled()`; or the Master controls `enabled` row becoming host-declared |

**Retired on 2026-09-08.** The register carried a `toc-file-§5` row for the within-`core/` file sequence, filed as `CM-49` in `docs/audits/2026-08-04/`. The section has since said what its MUST binds: the **section-header** order is the rule, the file sequence inside a section is a reference implementation, and an addon whose bootstrap forces a different one states the reason in a comment in the TOC itself — at which point the ordering is *compliant* and "needs no deviation-register row" (`toc-file.md:129`). Those comments sit in `ConsumableMaster.toc` itself: the `# Core` header comment ("Namespace.lua first …") and the one above `core\PerfSetup.lua` have been there since the row was written, and the one above `core\LifecycleSetup.lua` joined them on 2026-09-17; the row was recording a departure from a rule that no longer exists to depart from, which is the graveyard `documentation-§3` forbids. `CM-71` in `docs/audits/2026-09-07/` is the finding that says so.

**Retired on 2026-09-24.** The register carried a `preview-mode` row (no unlocked placeholder, no preview verb), decided 2026-08-05 on audit finding `CM-67`. Its trigger, a preview / test verb added to `/cm`, fired at `87ed204` (the macro bar's `/cm test` mode) and was reversed at `9c45eff`, and the row was never revisited. The rule has since moved too: from v2.49.0 the standard exempts an addon whose unlocked view is its preview and names Ka0s Consumable Master (`preview-mode.md:10`), so the row recorded a departure the current text no longer asks for. `CM-90` in `docs/audits/2026-09-23/` is the finding that says so.
...
**Nothing is over the cap today.** Both breaches this section was written for were peeled in 2026-09:
`tests/test_macrobar.lua` (2229) on the two cuts issue [#32](https://github.com/tusharsaxena/ConsumableMaster/issues/32) named, and
`tests/test_settingsui.lua` (2028) on the one cut issue [#33](https://github.com/tusharsaxena/ConsumableMaster/issues/33) named. Measured 2026-09-16 with
