...
**The launcher's click** follows `launcher-§2`: this addon is on **rung (b)** — its left click drives the macro bar's lock, which is its preview switch — so while disabled the left click prints the one refusal line, writes no SavedVariables and does nothing else — a refusal the launcher library makes (`LibKa0s-Launcher-1.0` minor 2's `isEnabled` / `disabledLine` descriptor gate, so the host's `onClick` is never reached) — while the right click opens the settings panel unchanged, in either state. **Its tooltip keeps drawing while disabled** (`launcher-§1`, `LibKa0s-Launcher-1.0` minor 3, which draws it from the descriptor): `Enabled: No`, `Locked: Yes|No`, and a left-click hint that reads `disabled — /cm enable` instead of the lock label; [settings-panel.md → the Minimap button row](./settings-panel.md#the-minimap-button-row--shown-says-one-thing-the-store-says-the-other) has the full shape.
