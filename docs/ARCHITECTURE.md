...
**Declared rows** are `KCM.Settings.Schema`, an ordered array published by `settings/Panel.lua` and appended to by the page files — **78** rows: 64 of them `macroBar.*`, 6 the General page's composed Master controls block (`options-ui-§15`), 7 on the Macros page and 1 on the Stat Priority page. Most are scalars. Ten are the whole-value rows described below, of type `order` or `map`, and one bool, `mouseover`. `settings/Panel.lua` itself declares none: a row is either written out in a page file or emitted by a LibKa0s composer and spliced in by `Helpers.RegisterRows`, which is also why the count is no longer greppable and is read off `#KCM.Settings.Schema`. Each row is `{ path = …, type = …, … }`, and one row is simultaneously three things: the widget on its settings page (its `group` is also the tab it lands on, so the array's order is the strip's order), the `/cm list|get|set|reset <path>` CLI entry (`settings/Slash.lua:333` hands the whole array to LibKa0s-Slash-1.0 as `allRows`), and the validator applied on write by the `Resolve` → `SetAndRefresh` seam.
...
An `order` validator drops unknown and repeated members and appends any missing ones. A flag map drops keys outside its member set and refuses a value that is not a real boolean. Both store a fresh table every time, so neither a caller's table nor a row's `default` (which is the `dbDefaults` table itself) ever lands in the profile. `/cm set` takes an order as comma-separated keys and a flag map as `KEY=on|off` pairs (`settings/Slash.lua`). `tests/test_schema.lua` fails if a runtime file writes one of these fields around the helper.

### Other state written outside the helper

Two pieces of persistent state are still written outside the helper, and neither is a setting. As of 2026-09-12 neither has a row or a register row:

- `profile.macroState[macroName]`, `MacroManager`'s per-macro fingerprint cache (`lastItemID` / `lastBody` / `lastIcon` / `lastCat`). It is created lazily in `MacroManager` (`modules/MacroManager.lua:465`), written per macro after each edit (`:486`), and wiped to `{}` by `InvalidateState` (`:636`, reached from `/cm rewritemacros` and the General page's Force rewrite macros button). It is data the addon learns, not a preference, so it gets no schema row. It is deliberately left as it is for now. The standard is adding a named class for non-setting state like this, and a later pass names this cache's owner module here once that lands. Until then it is the open remainder of [#35](https://github.com/tusharsaxena/ConsumableMaster/issues/35). The combat queue `pendingUpdates` lives only in memory, so it is out of scope;
- the macro bar's drag-written position `macroBar.point` / `.relPoint` / `.x` / `.y`, written on drag-stop (`modules/MacroBar.lua:100`) and by `MacroBar.ResetPosition` (`:483`), which the General page's reset-position button (`settings/General.lua:228-230`) and `/cm bar reset` (`core/SlashCommands.lua:803`) call. No schema row addresses these fields, and no control sets a chosen position: the reset puts back the shipped default. That makes this drag-only geometry, which is the MUST NOT's case and wants a `Documented deviations` row.

The Macro Bar page's **Defaults** button (`doResetPage`, `settings/MacroBar.lua`) writes every row on that page back to its default through `Helpers.SetManyAndRefresh`, the helper's batch form (`settings/Panel.lua`, published as `KCM.Schema:SetMany`). Each row is validated and written through `Helpers.Set`, and each logs its own `[Set]` line. The whole batch then re-applies the bar once and rebuilds the page once. `macroBar.locked` is the General page's row, so the button leaves it standing. Until 2026-09-12 the button replaced the whole `macroBar` table instead, which broke the schema-row MUST. That was [#36](https://github.com/tusharsaxena/ConsumableMaster/issues/36).
