...
Two files, measured 2026-09-13 with

```
git ls-files '*.lua' | grep -v '^libs/' | grep -v '^tests/_kit/' | xargs wc -l | sort -rn
```

| File | Lines (2026-09-13) | Disposition |
|---|---|---|
...
| `tests/test_settingsui.lua` | 1880 | Issue [#33](https://github.com/tusharsaxena/ConsumableMaster/issues/33) — one cut at `:833`, the three `options-ui` conformance blocks out to `test_settingsui_optionsui.lua` |
...
over the cap**. The largest are `settings/Panel.lua` at 1408, `settings/Category.lua` at 1237 and
...
**The 1000–1500 band is on notice, not in breach** (measured 2026-09-13): `settings/Panel.lua` (1408),
`settings/Category.lua` (1237), `settings/MacroBar.lua` (1166), `tests/test_schema.lua` (1012),
...
- **The macro bar's position.** Storage key: `profile.macroBar.point` / `.relPoint` / `.x` / `.y`, the bar's anchor against `UIParent`. It is geometry only a drag determines. **Owner:** `MacroBar` (`modules/MacroBar.lua`). **Writers:**
  - `savePosition` (`:94`, write at `:100`) reads the anchor back off the frame when a drag stops, on the bar itself (`OnDragStop`, `:136-139`) or on its drag handle (`:164-167`).
  - `MB.ResetPosition` (`:479`, write at `:483`) puts back the shipped default from `KCM.dbDefaults`. Three acts reach it: the General page's **Reset position** button (`onResetPosition`, `settings/General.lua:214`), `/cm bar reset` (`core/SlashCommands.lua:861`) and the Macro Bar page's **Defaults** (`doResetPage`, `settings/MacroBar.lua:638`).
...
Every client event this addon listens to is registered in one place — `KCM:OnEnable`, `core/ConsumableMaster.lua:639-648` — through AceEvent. No module body subscribes on its own, so the whole surface is readable at a glance.
...
- **Options registration is deferred in combat; opening is refused outright.** `Settings.RegisterAddOnCategory` is protected, and registering under lockdown taints the Settings window for the rest of the session — so `settings/Panel.lua:1162-1164` parks the bootstrap behind `KCM.Settings.registerPending` and the addon's existing `PLAYER_REGEN_ENABLED` handler replays it, rather than a second event registration of the panel's own. Nothing is lost by waiting, because the panel cannot be opened mid-fight anyway: the *open* path — `settings/Panel.lua:1346-1349` — turns a mid-fight `/cm config` into a chat notice instead of a silent failure, as does the panel's Defaults action, `settings/Panel.lua:459-461`.
...
| `Item-1.0` | `core/ItemSetup.lua` | exactly one of the major's four members, `ItemIDFromLink`, behind the Add-by-ID line's pasted-link path (`settings/Category.lua:494`). `QualityFromLink` / `QualityLabel` / `LoadItem` are pointedly not taken, and `core/TooltipCache.lua` stays this addon's own policy. The degradation stub is the same primitive in three lines, so a pasted link never works on one install and not another |
