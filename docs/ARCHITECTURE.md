...
- **The macro bar's position.** Storage key: `profile.macroBar.point` / `.relPoint` / `.x` / `.y`, the bar's anchor against `UIParent`. It is geometry only a drag determines. **Owner:** `MacroBar` (`modules/MacroBar.lua`). **Writers:**
  ...
  - `MB.ResetPosition` (`:534`, write at `:538`) puts back the shipped default from `KCM.dbDefaults`. Three acts reach it: the General page's **Reset position** button (`onResetPosition`, `settings/General.lua:216`), `/cm bar reset` (`core/SlashCommands.lua:939`) and the Macro Bar page's **Defaults** (`doResetPage`, `settings/MacroBar.lua:635`).
