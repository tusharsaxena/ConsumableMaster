...
Every client event this addon listens to is registered in one place — `KCM:OnEnable`, `core/ConsumableMaster.lua:558-566` — through AceEvent. No module body subscribes on its own, so the whole surface is readable at a glance.
