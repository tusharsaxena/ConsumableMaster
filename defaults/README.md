...
4. **Check for new stat-buff phrasings.** The TooltipCache parser matches specific stat names ("Critical Strike", "Haste", etc.) AND the wildcard phrasing `"<amount> of your highest secondary stat"`. If a new potion reads differently (e.g. `"of your primary stat"` or `"of a random secondary stat"`), extend `parseStatBuffs` in `core/TooltipCache.lua` and the corresponding special case in `modules/Ranker.lua`. The wildcard token (`TOP_SECONDARY`) is resolved in `secondaryWeight`, which `statWeight` delegates to for every non-primary stat; a new PRIMARY-side alias token goes in the module-level `PRIMARY_ALIAS` table that `statWeight` reads.
